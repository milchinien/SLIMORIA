...
// Zweiter Renderpfad (Phase Grafik). Muss in der Adresse stehen: der Renderer
// wird einmal beim Laden der Seite gebaut, ein spaeteres Umschalten kaeme zu spaet.
const rendererFlag = flag('renderer', null);
...
async function openGame(browser, port, rendererFlag) {
  ...
  const adresse = `http://127.0.0.1:${port}/client/index.html?capture=1`
    + (rendererFlag ? `&renderer=${rendererFlag}` : '');
...
try {
  const { page, fehler } = await openGame(browser, port, rendererFlag);
